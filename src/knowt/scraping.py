...
BASE_DOMAIN = 'www.openwall.com'
LIST = 'lists/oss-security'
PATH = f'{LIST}/{DATE.year:04d}/{DATE.month:02d}/{DATE.day:02d}'
DEFAULT_URL = '/'.join([f'https://{BASE_DOMAIN}', PATH])

RE_URL = r'^(?P<scheme>[a-zA-Z]{2,6}:/{2,3})?(?P<host>[-\w]+([.][-\w]+){1,9})?(?P<path>[/]*[.]*.+)?$'


#####################################################################################
# create python objects from YAML/JSON strings
...
def scrape_links(
    ...
    """ Extract the a href urls from a page and return a DataFrame of absolute URLs and metadata

    >>> df = scrape_links()
    >>> df.shape
    (62, 6)
    >>> list(df.columns)
    ['scheme', 'host', 'path', 'url', 'text', 'absolute_url']
    """
    urlgroups = re.match(RE_URL, url).groupdict()
    ...
    for a in bs.find_all("a"):
        href = a.get("href", "")
        # stripped_href = href.strip(".").strip("/")
        match = pattern.match(href)
        if match:
            groups = re.match(RE_URL, href).groupdict()
            row = dict(
                scheme=groups['scheme'] or urlgroups['scheme'],
                host=groups['host'] or urlgroups['host'],
                path=groups['path'] or urlgroups['path'],
                url=href,
                text=a.text,
                # href2=a.next_sibling.next_sibling["href"],
                # text2=a.next_sibling.next_sibling.text,
            )
            row['absolute_url'] = row['scheme'] + row['host'] + row['path']
        links.append(row)

    df = pd.DataFrame(links)
    # df = df.sort_values("absolute_url")
    # df["text"].str.split("::").str[-1]
    return df
...
def scrape_index_ignore_twat(url=f"https://{BASE_DOMAIN}/eps/index.html"):
    ...
    episodes = [[parse_hpr_url(e[0]).get("url", "")] + e for e in episodes]
...
def clean_hpr_df(df, base_url_regex=BASE_URL_REGEX):
...
def normalize_hpr_url(url, host_regex=BASE_URL_REGEX, normalized_host=""):
...
def update_hpr(update_existing=False, base_domain=BASE_DOMAIN, host_regex=BASE_URL_REGEX):
    ...
    for i, row in tqdm(dflatest.iterrows()):
        ...
        url = normalize_hpr_url(row["url"], "").strip().strip(".").strip("/")
    ...
    # reclean in case additional URLs have been added to df
    dflatest = clean_hpr_df(dflatest)
